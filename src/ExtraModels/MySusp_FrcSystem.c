...
static const char ThisModelClass[] = "Susp_FrcSystem";
...
static void *
MyModel_New (struct tInfos *Inf, struct tSusp_FrcSystemCfgIF *CfgIF, const char *KindKey)
{
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_FrcSystem, KindKey,
...
static int
MyModel_Calc (void *MP, tSusp_FrcSystemIF *IF, double dt)
{
    struct tMyModel *mp = (struct tMyModel*)MP;
    int	iS;
...
Susp_FrcSystem_Register_MyModel (void)
{
    ...
    memset (&m, 0, sizeof(m));
    m.Susp_FrcSystem.VersionId =	ThisVersionId;
    m.Susp_FrcSystem.New =		MyModel_New;
    m.Susp_FrcSystem.Calc =		MyModel_Calc;
    m.Susp_FrcSystem.Delete =		MyModel_Delete;
    m.Susp_FrcSystem.DeclQuants =	MyModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.Susp_FrcSystem.ParamsChanged = 	ParamsChanged_IgnoreCheck;
