...
static const char ThisModelClass[] = "Susp_DamperSystem";
...
static void *
MyModel_New (struct tInfos *Inf, struct tSusp_DamperSystemCfgIF *CfgIF, const char *KindKey)
{
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_DamperSystem, KindKey,
...
static int
MyModel_Calc (void *MP, tSusp_DamperSystemIF *IF, double dt)
{
    struct tMyModel *mp = (struct tMyModel*)MP;
    int	iS;
...
Susp_DamperSystem_Register_MyModel (void)
{
    ...
    memset (&m, 0, sizeof(m));
    m.Susp_DamperSystem.VersionId =	ThisVersionId;
    m.Susp_DamperSystem.New =		MyModel_New;
    m.Susp_DamperSystem.Calc =		MyModel_Calc;
    m.Susp_DamperSystem.Delete =	MyModel_Delete;
    m.Susp_DamperSystem.DeclQuants =	MyModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.Susp_DamperSystem.ParamsChanged =	ParamsChanged_IgnoreCheck;
